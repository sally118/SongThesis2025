...
# Lysate sample measurements
lysates <- read_excel("/Users/sallysong/Downloads/LDOPA.xlsx", sheet = "0319_Lysates")

## Rows 1,3 are replicates of each other.
## Rows 2,4 are replicates of each other (0.5 dilution).

lysates <- lysates[-c(2, 4), ] # Remove 0.5 dilution values, messes with LDOPA conc later

## Subtract the background (FM) mean abs from the samples

lysates$Non <- lysates$Non - mean(lysates$FM)
lysates$`Non (IPTG)` <- lysates$`Non (IPTG)` - mean(lysates$FM)
lysates$`Non (IPTG+tyr)` <- lysates$`Non (IPTG+tyr)` - mean(lysates$FM)
lysates$LD <- lysates$LD - mean(lysates$FM)
lysates$`LD (IPTG)` <- lysates$`LD (IPTG)` - mean(lysates$FM)
lysates$`LD (IPTG+tyr)` <- lysates$`LD (IPTG+tyr)` - mean(lysates$FM)

lysates_long <- lysates %>%
  pivot_longer(cols = matches("^(Non|LD|FM)"), 
               names_to = "Treatment", 
               values_to = "Absorbance")

lysates_long <- lysates_long[lysates_long$Treatment != "FM", ]
...
### Predict LDOPA conc from sample lysates

lysates_long <- lysates_long %>%
  #mutate(pred_LDOPA = predict(model, newdata = data.frame(Absorbance)))
  #changing predicting function
  mutate(pred_LDOPA = ED(object = model, lysates_long$Absorbance, type = "absolute", display = FALSE, multcomp = FALSE)[, 1])

lysates_long$pred_LDOPA[is.nan(lysates_long$pred_LDOPA)]<-0
...
summ_lysates_long <- lysates_long %>%
  group_by(Treatment) %>%
  summarize(
    mn = mean(pred_LDOPA),
    std = sd(pred_LDOPA),
...
summ_lysates_long <- summ_lysates_long %>%
  mutate(
    Strain = factor(case_when(
      str_starts(Treatment, "Non") ~ "Non",
      str_starts(Treatment, "LD") ~ "LD",
...
ggplot(summ_lysates_long, aes(x = Treatment, y = mn, fill=fill_color)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6) + 
  geom_errorbar(aes(ymin = pmax(ymin,0), ymax = pmax(ymax,0)), 
                width = 0.2, position = position_dodge(width = 0.7)) +
  geom_point(data = lysates_long, 
             aes(x = Treatment, y = pred_LDOPA, Strain = Strain), 
...
kruskal.test(pred_LDOPA ~ Treatment, data = lysates_long)

NonKruskal <- lysates_long %>%
  filter(Strain == "Non")
LDKruskal <- lysates_long %>%
  filter(Strain == "LD")

kruskal.test(pred_LDOPA ~ Treatment, data = NonKruskal)
kruskal.test(pred_LDOPA ~ Treatment, data = LDKruskal)
